...
//Кастомные функции
function add_element_primary_menu(){
	include $_SERVER['DOCUMENT_ROOT'].'/includes/add-element-primary-menu.php';
}
add_action("wp_print_footer_scripts", "add_element_primary_menu", 1000);
...
function is_page_lk(){
	if(empty($_SESSION["user_id"])){
		...
		$url = explode('?', $url);
		if($url == "lk"){
			wp_redirect(((!empty($_SERVER['HTTPS'])) ? 'https' : 'http')."://".$_SERVER["HTTP_HOST"]);
		}
	}
}
add_action("init", "is_page_lk");
